...
상태 어휘는 `packages/types/src/labels.ts` 가 단일 원천입니다. 종류, 보호 상황, 진행 상태는 서로 다른 축이라 한 목록으로 합치지 않고, 카드에는 `reportStatus` 가 고른 배지 하나만 붙입니다. 배회 중, 찾음 처럼 누가 그런지 해석해야 하는 말은 쓰지 않고, 한 종류만 담는 탭 안에서는 탭 이름을 배지로 되풀이하지 않습니다. 폼에서 고르는 문구와 저장 뒤 배지의 끝말은 같아야 합니다.
