...
- [x] `DECK-1` 부터 `DECK-5` 를 제출 슬롯 5장에 맞춰 재구성해 다시 굽기. 장당 목업 3개 이상
...
- [x] 새 덱과 새 캡처에 `배회 중` `찾는 중` 등 Phase 3 금지 어휘가 남지 않았는지 눈으로 확인하기
...
- 제출 덱은 슬롯 5장 제한에 맞춰 8장에서 5장으로 재구성. 캡처 23장을 전부 배치하고 장당 목업 3개 이상
- 덱 말투는 `[flex]서비스 소개서.pdf` 의 화법을 따름. 구어 종결과 수사 의문문을 섞고 대구 문장·번역투를 뺌
- `HERO-hero-16x9.png` 는 gpt-image 로 만든 동네 일러스트 위에 Pretendard 로 문구를 얹음. 사진을 쓰지 않음
- DECK-5 의 `좌표를 읽는 코드가 0줄` 표현이 사실과 달라 `정확 좌표는 서버 안에서만 쓰고 공개 응답에는 300m 격자만` 으로 고침
